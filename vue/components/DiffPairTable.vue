                ...
                <tr v-for="item in data" :key="getRowKey(item)">
                    <td v-for="col in displayColumns" :key="col.key" :style="{ width: col.width }">
                        ...
                        <template v-else-if="col.key === '__first__' && actions">
                            <button v-for="action in actions" :key="action.key" class="action-btn" :class="action.class"
                                :disabled="getActionDisabled(item, action)" @click="() => onActionClick(item, action)">
